...
class ShingleGenerator(Tokenizer):
    ...
    def spans(self, buffer: str) -> Iterator[Tuple[int, int]]:
        ...
        # Returnerer ingenting hvis bufferen er tom
        buffer_len = len(buffer)
        if buffer_len == 0:
            return

        # Hvis bufferen er kortere enn eller lik shingle-bredden, returner hele bufferen som én shingle
        if buffer_len <= self.__width:
            yield (0, buffer_len)
            return

        # Generer overlappende shingler med lengde lik shingle-bredden
        for start in range(buffer_len - self.__width + 1):
            yield (start, start + self.__width)
